...
import { TARGET_CHAIN, TARGET_CHAIN_CONFIG } from '../config/wagmi';
...
export function WalletButton() {
  ...
  }, [switchChain, addAndSwitchNetwork]);

  useEffect(() => {
    if (isConnected && chain && !isOnCorrectChain && !isSwitching && !isAddingNetwork) {
      handleSwitchChain();
    }
  }, [isConnected, chain, isOnCorrectChain, isSwitching, isAddingNetwork, handleSwitchChain]);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <div className={`w-2 h-2 rounded-full shadow-sm ${isOnCorrectChain ? 'bg-green-400 shadow-green-400/50' : 'bg-red-400 shadow-red-400/50'}`} />
...
function DisconnectIcon({ className = '' }: { className?: string }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 ${className}`}>
